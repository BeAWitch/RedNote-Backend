...
@Slf4j
@Service("openAITranslationAdapter")
@RequiredArgsConstructor
public class OpenAITranslationAdapter implements IAiTranslationService {
    ...
    public AIResponseVO translate(AITranslationRequestDTO request) {
        log.info("OpenAI 文本翻译服务调用开始 - 用户ID: {}", request.getUserId());
        ...
        try {
            ...
            log.info("OpenAI 文本翻译服务调用结束 - 用户ID: {}", request.getUserId());
        }
    }
    ...
    private AIResponseVO buildSuccessResponse(ChatResponse chatResponse, Instant startTime, AITranslationRequestDTO request) {
        ...
        log.info("OpenAI 文本翻译调用成功 - 用户ID: {}, 响应时间: {}ms, 翻译内容长度: {}",
